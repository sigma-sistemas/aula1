...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['atualizar_demanda'])) {
    ...
    if (isset($_FILES['arquivo_anexo']) && $_FILES['arquivo_anexo']['error'] == 0) {
        $arquivo_nome = $_FILES['arquivo_anexo']['name'];
        $arquivo_tmp = $_FILES['arquivo_anexo']['tmp_name'];
    ...
    if ($conn->query($update_sql) === TRUE) {
        ...
        $historico_sql = "INSERT INTO historico_demandas (demanda_id, data_atualizacao, responsavel, comentario, arquivo, tipo_arquivo) 
                          VALUES ('$demanda_id', '$data_atualizacao', '$novo_responsavel', '$comentario', '$destino', '$extensao')";
...
<body>
    <div class="form-container">
        ...
        <form action="atualizar_demanda.php?id=<?php echo $demanda_id; ?>" method="POST" enctype="multipart/form-data">
            ...
            <label for="arquivo_anexo">Anexar Arquivo (PDF, JPG, PNG):</label>
            <input type="file" id="arquivo_anexo" name="arquivo_anexo" accept=".pdf,.jpg,.png">
